...
namespace Jering.JavascriptUtils.Node.HostingModels
{
    ...
    public abstract class OutOfProcessNodeInstance : INodeInstance
    {
        ...
        private const string ConnectionEstablishedMessage = "[Jering.JavascriptUtils.Node:Listening]";
        private readonly TaskCompletionSource<object> _connectionIsReadySource = new TaskCompletionSource<object>();
        private bool _disposed;
        private readonly string _entryPointScript;
        private FileSystemWatcher _fileSystemWatcher;
        private readonly int _invocationTimeoutMilliseconds;
        ...
        {
            OutputLogger = nodeOutputLogger ?? throw new ArgumentNullException(nameof(nodeOutputLogger));

            _entryPointScript = entryPointScript;
            _invocationTimeoutMilliseconds = invocationTimeoutMilliseconds;

            ProcessStartInfo startInfo = PrepareNodeProcessStartInfo(
                _entryPointScript.FileName,
                projectPath,
            ...
            CancellationToken cancellationToken, string moduleName, string exportNameOrNull, params object[] args)
        {
            if (_nodeProcess.HasExited || _nodeProcessNeedsRestart)
            {
                ...
                string message = _nodeProcess.HasExited
            ...
            using (CancellationTokenSource combinedCancellationTokenSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, timeoutSource.Token))
            {
                ...
                bool connectionDidSucceed = false;
                    ...
                    if (timeoutSource.IsCancellationRequested)
                    {
                        ...
                        else
                        {
                            ...
                            throw new NodeInvocationException(
                                ...
                                $"You can change the timeout duration by setting the {HttpNodeInstanceOptions.TimeoutConfigPropertyName} "
                                + $"property on {nameof(HttpNodeInstanceOptions)}.\n\n"
        ...
        /// <returns></returns>
        protected virtual ProcessStartInfo PrepareNodeProcessStartInfo(string entryPointFilename, string projectPath, string commandLineArguments, IDictionary<string, string> environmentVars)
        {
            // This method is virtual, as it provides a way to override the NODE_PATH or the path to node.exe
            int thisProcessPid = Process.GetCurrentProcess().Id;
            ...
            string nodePathValue = existingNodePath + Path.Combine(projectPath, "node_modules");
        ...
        private bool IsFilenameBeingWatched(string fullPath)
        {
            ...
            {
                string actualExtension = Path.GetExtension(fullPath) ?? string.Empty;
