...
| EnableFileWatching | `bool` | The value specifying whether file watching is enabled. If file watching is enabled, when a file in `WatchPath` with name matching a pattern in `WatchFileNamePatterns` changes, NodeJS is restarted. | `0` |
...
How long it takes for NodeJS to restart and begin accepting invocations:
